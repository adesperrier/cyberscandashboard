...
function showDetails(portJson) {
    ...
    modalBody.innerHTML = `
        <div class="modal-item">
            ...
            <div class="modal-value">${port.port}</div>
        </div>
        <div class="modal-item">
            <div class="modal-label">State</div>
            <div class="modal-value"><span class="status-badge status-${port.state}">${port.state}</span></div>
        </div>
        <div class="modal-item">
            <div class="modal-label">Service</div>
            <div class="modal-value">${port.service}</div>
        </div>
        <div class="modal-item">
            <div class="modal-label">Product</div>
            <div class="modal-value">${port.product || 'Unknown'}</div>
        </div>
        <div class="modal-item">
            <div class="modal-label">Version</div>
            <div class="modal-value">${port.version || 'Unknown'}</div>
        </div>
        <div class="modal-item">
            <div class="modal-label">Risk Level</div>
            <div class="modal-value"><span class="status-badge risk-${port.risk.toLowerCase()}">${port.risk}</span></div>
        </div>
        ...
        history.forEach(item => {
            ...
            element.innerHTML = `
                ...
                <div class="history-summary">
                    <div><span class="history-stat">${item.summary.open_ports}</span> open</div>
                    <div><span class="history-stat">${item.summary.total_ports}</span> total</div>
                    <div style="color: #ff006e;">${item.summary.critical_issues}</div> critical
                    <div style="color: #ff4400;">${item.summary.high_issues}</div> high
                </div>
            `;
